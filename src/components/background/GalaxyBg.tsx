...
vec3 StarLayer(vec2 uv) {
  ...
  for (int y = -1; y <= 1; y++) {
    for (int x = -1; x <= 1; x++) {
      ...
      // Yellow (#EDB600) or white — 65% yellow, 35% white
      float colorSeed = Hash21(si + 7.0);
      vec3 yellow = vec3(0.929, 0.714, 0.0);
      vec3 white  = vec3(1.0, 1.0, 1.0);
      vec3 base = mix(yellow, white, step(0.65, colorSeed));
...
export default function GalaxyBg() {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 0, pointerEvents: 'none', background: '#000' }}>
      <div ref={ctnRef} style={{ width: '100%', height: '100%', position: 'absolute' }} />
    </div>
  )
}
